...
export async function POST(req: NextRequest) {
  ...
  if (action === "delete") {
    // Remove dependent rows first (FK cascade isn't guaranteed over libSQL).
    await db
      .prepare(`DELETE FROM call_logs WHERE lead_id IN (${placeholders})`)
      .run(...ids);
    await db
      .prepare(`DELETE FROM activities WHERE lead_id IN (${placeholders})`)
      .run(...ids);
    const info = await db
